 ...
 * @param {any} marcaFromDb - The raw marca object from MongoDB (where _id is ObjectId).
 * @returns {MarcaVehiculo} The serialized marca object (where _id and idModelo are strings).
 */
function serializeMarca(marcaFromDb: any): MarcaVehiculo { 
  return {
    ...marcaFromDb,
    // Ensure _id is a string. Manager methods already do this, but this is a safeguard.
 ...
 * The `idModelo` is generated by the manager as an ObjectId string and returned.
...
export async function getModelosByMarcaAction(marcaId: string): Promise<ActionResult<Pick<ModeloVehiculo, 'idModelo' | 'modelo'>[]>> {
 ...
 try {
    ...
    const marca = await marcaManager.getMarcaById(marcaId); // Manager returns _id and idModelo as strings.
