...
export default function DashboardPage() {
  ...
  /**
   * Actualiza el estado `newOrderData` o `editOrderData` al cambiar un checkbox.
   * @param name Nombre del campo del checkbox.
  ...
  /** Maneja la creación de un nuevo empleado y opcionalmente su usuario de sistema. */
  const handleCreateEmpleado = async () => {
    ...
    if (result.success) {
      toast({ title: "Éxito", description: `Empleado "${nombre}" creado.` });
  ...
  /** Maneja la actualización de un empleado y/o sus credenciales. */
   ...
   * @param name Nombre del campo (usado para el estado).
   ...
   * @param isTextarea Indica si es un Textarea.
   * @param isDisabled Indica si el campo está deshabilitado.
   * @param isRequired Indica si el campo es obligatorio (solo visual, la validación es separada).
   */
  const renderDialogField = (
    ...
    return (
      <div className="space-y-1">
        <Label htmlFor={name} className="text-sm font-medium">{label}{isRequired && <span className="text-destructive">*</span>}</Label>
        {type === 'select' && options ? (
          <Select name={name} onValueChange={handleSelect} value={value || ''} disabled={isDisabled}>
            <SelectTrigger id={name} className="w-full mt-1"><SelectValue placeholder={placeholder || "Seleccionar..."} /></SelectTrigger>
            <SelectContent>
