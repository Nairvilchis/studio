...
import type { Service, ServicesSectionContentData } from '@/lib/types';
import ServiceCard from './service-card';
import { readServices, readServicesSectionContent } from '@/lib/actions';
...
export default async function ServicesSection() {
  ...
  // Cargar contenido de la sección (título y descripción)
  const contentResponse = await readServicesSectionContent();
  let sectionContent: ServicesSectionContentData;

  if (contentResponse.success && contentResponse.data) {
    sectionContent = contentResponse.data;
  } else {
    console.warn("Failed to load services section content, using fallback data. Message:", contentResponse.message);
    sectionContent = {
